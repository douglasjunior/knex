...
var _saveAsyncStack = _interopRequireDefault(
  require('../util/save-async-stack')
);
...
const validateWithArgs = function validateWithArgs(alias, statement, method) {
  ...
  if (
    ...
    statement instanceof _raw.default
  ) {
    return;
  }
...
(0, _lodash.assign)(Builder.prototype, {
  ...
  where(column, operator, value) {
    ...
    if (column instanceof _raw.default && arguments.length === 1)
      return this.whereRaw(column); // Allows `where({id: 2})` syntax.

    if ((0, _lodash.isObject)(column) && !(column instanceof _raw.default))
      return this._objectWhere(column); // Enable the where('key', value) syntax, only when there
  ...
  whereRaw(sql, bindings) {
    const raw =
      sql instanceof _raw.default ? sql : this.client.raw(sql, bindings);
