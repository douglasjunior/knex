...
var _parseConnection = _interopRequireDefault(
  require('./util/parse-connection')
);
...
function Knex(config) {
  ...
  if (typeof config === 'string') {
    const parsedConfig = Object.assign(
      (0, _parseConnection.default)(config),
      arguments[2]
    );
  ...
  if (arguments.length === 0 || (!config.client && !config.dialect)) {
    ...
    config.client.prototype instanceof _client.default
  ) {
    ...
    Dialect = require(`./dialects/${_constants.CLIENT_ALIASES[clientName] ||
      clientName}/index.js`);
