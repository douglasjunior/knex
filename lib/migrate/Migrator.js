...
var migrationListResolver = _interopRequireWildcard(
  require('./migration-list-resolver')
);

var _fsMigrations = _interopRequireWildcard(require('./sources/fs-migrations'));

var _MigrationGenerator = _interopRequireDefault(
...
class Migrator {
  constructor(knex) {
    ...
    this.generator = new _MigrationGenerator.default(
      knex.client.config.migrations
    );
    ...
    return migrationListResolver
      ...
      .spread((all, completed) => {
        ...
        const transactionForAll =
          ...
          (0, _lodash.isEmpty)(
            (0, _lodash.filter)(migrations, (migration) => {
    ...
    return migrationListResolver
      ...
      .then((completed) => {
        const val = (0, _lodash.max)(
          completed.map((value) => value.split('_')[0])
        );
  ...
  forceFreeMigrationsLock(config) {
    this.config = getMergedConfig(config, this.config);
    const lockTable = (0, _tableResolver.getLockTableName)(
      this.config.tableName
    );
  ...
  _isLocked(trx) {
    const tableName = (0, _tableResolver.getLockTableName)(
      this.config.tableName
    );
    ...
    (0, _lodash.each)(migrations, (migration) => {
      ...
      current = current
        .then(() => {
          ...
          return warnPromise(
            this.knex,
            migrationContent[direction](trxOrKnex, _bluebird.default),
            name
          );
        })
        .then(() => {
  ...
  _transaction(migrationContent, direction, name) {
    return this.knex.transaction((trx) => {
      return warnPromise(
        this.knex,
        migrationContent[direction](trx, _bluebird.default),
        name,
        () => {
          trx.commit();
        }
...
function getMergedConfig(config, currentConfig) {
  ...
  // applied to it.
  const mergedConfig = (0, _lodash.assign)(
    {},
    CONFIG_DEFAULT,
    currentConfig || {},
    config
  ...
  if (
    config && // If user specifies any FS related config,
  ...
  if (!mergedConfig.migrationSource) {
    mergedConfig.migrationSource = new _fsMigrations.default(
...
function getMissingMigrations(migrationSource, completed, all) {
  return (0, _lodash.differenceWith)(
    completed,
    all,
    (completedMigration, allMigration) => {
      return (
...
function getNewMigrations(migrationSource, all, completed) {
  return (0, _lodash.differenceWith)(
    all,
    completed,
    (allMigration, completedMigration) => {
      return (
        completedMigration === migrationSource.getMigrationName(allMigration)
      );
    }
