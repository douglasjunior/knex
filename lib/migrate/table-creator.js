...
function ensureTable(tableName, schemaName, trxOrKnex) {
  ...
  const lockTableWithSchema = (0, _tableResolver.getLockTableNameWithSchema)(
    tableName,
    schemaName
  );
  return getSchemaBuilder(trxOrKnex, schemaName)
    ...
    .then(() =>
      (0, _tableResolver.getTable)(trxOrKnex, lockTable, schemaName).select('*')
    )
    .then(
      (data) =>
        !data.length &&
        trxOrKnex.into(lockTableWithSchema).insert({
          is_locked: 0,
        })
    );
}
